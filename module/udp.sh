...
limiter() {
  ltr() {
    ...
    for i in $(atq | awk '{print $1}'); do
      if [[ ! $(at -c $i | grep '/root/udp/limiter.sh') = "" ]]; then
        atrm $i
        sed -i '/root/udp/limiter.sh/d' /var/spool/cron/crontabs/root
  ...
  l_exp() {
    ...
    if [[ -z "$l_cron" ]]; then
      print_center -ama '0 1 * * * /root/udp/limiter.sh --ssh'>>/var/spool/cron/crontabs/root
  ...
  log() {
    ...
    msg -bar
    [[ ! -e /root/udp/limit.log ]] && touch /root/udp/limit.log
    if [[ -z $(cat /root/udp/limit.log) ]]; then
      print_center -ama "${a85:-no account limiter record}"
      msg -bar
      sleep 2
      return
    fi
    msg -teal "$(cat /root/udp/limit.log)"
    msg -bar
    print_center -ama "►► ${a86:-Press enter to continue...} ◄◄"
    print_center -ama "►► ${a87:-0 to clear record} ◄◄"
    read option
    [[ $option = "0" ]] && print_center -ama "">/root/udp/limit.log
  }
...
Uninstall_UDP_CUSTOM() {
  ...
  rm -rf /root/udp
  rm -rf /usr/bin/udp
  systemctl stop udpgw.service &>/dev/null
...
reset_udp_custom() {
  if [[ $(systemctl is-active custom-server.service) = 'active' ]]; then
    systemctl stop custom-server.service &>/dev/null
    systemctl disable custom-server.service &>/dev/null
    ...
    if [[ $(systemctl is-active custom-server.service) = 'active' ]]; then
      ...
      systemctl start custom-server.service &>/dev/null
      print_center -verd "${a36:-UDP CUSTOM Running!}"
    else
      print_center -verm2 "${a37:-failed to start request-server.service!}"
    fi
  fi
  enter
}
...
quit_exclude() {
  ...
  exclude=$(cat /etc/systemd/system/custom-server.service | grep 'exclude' | awk '{print $4}')
  ports=($port)
  ...
  if [[ $option = $a ]]; then
    sed -i "s/$exclude /" /etc/systemd/system/custom-server.service
    ...
    sed -i "s/$exclude/-exclude=$Port/" /etc/systemd/system/custom-server.service
  fi
  if [[ $iniciar = 1 ]]; then
    systemctl enable custom-server.service &>/dev/null
    systemctl start custom-server.service &>/dev/null
...
vps_info() {
  ...
  clear
  title_sub "\033[3;40m${a10:-•UDP CUSTOM} Resleeved Net"
...
# [UDP - CUSTOM]
Menu_UDP_CUSTOM() {
  title_sub "\033[3;40m${a10:-• UDP CUSTOM} •"
  print_center -ama 'Release: v.5'
  ...
  msg -bar3
  echo -e " $(msg -verd "[1]") $(msg -verm2 '>') $(msg -azu "${a2:-Turn On/Off UDP}") $state"
...
# [MAIN MENU]A
menu_main() {
  main_title "\033[3;40m${a10:-• UDP CUSTOM} •"
  print_center -ama 'Release: v.5'
  print_center -ama 'Resleeved Net'
  msg -bar
  ...
  msg -bar3
  echo " $(msg -verd "[2]") $(msg -verm2 '>') $(msg -teal "${a10:-VPS Info}")"
  msg -bar3
  echo " $(msg -verd "[3]") $(msg -verm2 '>') $(msg -verm2 "${a3:-Uninstall UDP CUSTOM}")"
  exit2home
